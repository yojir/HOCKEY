...
const draw = () => {
    ...
    if(y +dy < 0 + ballRadius + paddleHeightE && (x +dx >= paddleXEW && x +dx <= paddleXEW + paddleWidthE)){
    ...
    if(y +dy > hi - ballRadius - paddleHeightM && (x + dx >= paddleXMW && x + dx <= paddleXMW + paddleWidthM)){
        dy = -(Math.random()/8 + 1 )*dy;
        dx = (Math.random()/8 + 1 )*dx;
        score++;
    }
